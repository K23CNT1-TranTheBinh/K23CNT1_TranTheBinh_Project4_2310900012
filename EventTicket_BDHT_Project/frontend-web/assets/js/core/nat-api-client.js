...
function resolveAppUrl(path) {
    ...
    if (appRootRelative) {
        ...
        // Ví dụ: trang ở /pages/index.html, nếu trả về 'components/nat-header.html'
        // trình duyệt sẽ fetch /pages/components/nat-header.html (SAI).
        // Phải trả về '/components/nat-header.html' để fetch đúng.
...
function normalizeHeaderLocations(events = []) {
    ...
    events.forEach((event) => {
        appendUnique(event?.venue?.city || extractCityFromAddress(event?.venue?.address || event?.location));
    });
    ...
    async request(endpoint, options = {}) {
        ...
        try {
            ...
            if (response.status === 401) {
                this.clearToken();
                window.location.href = window.pageUtils.resolveUrl('pages/user/nat-login.html');
                return null;
            }
...
window.pageUtils = {
    ...
    async loadHeader() {
        ...
        this._headerPromise = (async () => {
            try {
                const response = await fetch(this.resolveUrl('components/nat-header.html'));
    ...
    async loadFooter() {
        ...
        this._footerPromise = (async () => {
            try {
                const response = await fetch(this.resolveUrl('components/nat-footer.html'));
                if (!response.ok) throw new Error(`HTTP ${response.status}`);
                ...
                if (!document.querySelector('script[src*="nat-ai-chat.js"]')) {
                    const script = document.createElement('script');
                    script.src = this.resolveUrl('assets/js/nat-ai-chat.js');
    ...
    async populateHeaderNavigation() {
        ...
        allLink.href = this.resolveUrl('pages/user/nat-all-events.html');
        ...
        finalCategories.forEach((category) => {
            ...
            link.href = `${this.resolveUrl('pages/user/nat-all-events.html')}?category=${encodeURIComponent(category)}`;
        ...
        newsLink.href = this.resolveUrl('pages/user/nat-news.html');
    ...
    async populateHeaderLocationFilter() {
        ...
        select.innerHTML = '<option value="" style="color: #1e293b; background-color: #ffffff;">Mọi địa điểm</option>';
        ...
        select.addEventListener('change', () => {
            const isAllEventsPage = window.location.pathname.includes('/pages/user/nat-all-events.html');
            ...
            const targetPath = isAllEventsPage ? 'pages/user/nat-all-events.html' : 'pages/index.html';
