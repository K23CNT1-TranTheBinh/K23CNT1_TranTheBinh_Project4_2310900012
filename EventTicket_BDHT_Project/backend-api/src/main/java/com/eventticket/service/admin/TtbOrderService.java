...
public class TtbOrderService {
    ...
    // ==========================================
    public List<G8_order> getOrders(String keyword, Integer userId, String status, LocalDateTime startDate, LocalDateTime endDate) {
        
        // Ưu tiên 1 (MỚI): Nếu có từ khóa tìm kiếm (nhập email hoặc tên khách)
        if (keyword != null && !keyword.trim().isEmpty()) {
            return orderRepository.searchOrdersByKeywordAndStatus(keyword, status);
        }

        // Ưu tiên 2: Lọc theo khoảng thời gian
        if (startDate != null && endDate != null) {
            return orderRepository.findOrdersByDateRange(startDate, endDate);
        }
        
        // Ưu tiên 3: Lọc kết hợp khách hàng và trạng thái
        if (userId != null && status != null && !status.trim().isEmpty()) {
            return orderRepository.findByUserIdAndStatus(userId, status);
        }
        
        // Ưu tiên 4: Lọc riêng theo khách hàng
        if (userId != null) {
            return orderRepository.findByUser_UserId(userId);
        }
        
        // Ưu tiên 5: Lọc riêng theo trạng thái đơn
        if (status != null && !status.trim().isEmpty()) {
            return orderRepository.findByStatus(status);
        }
        
        // Mặc định: Trả về tất cả nếu không truyền tham số nào
