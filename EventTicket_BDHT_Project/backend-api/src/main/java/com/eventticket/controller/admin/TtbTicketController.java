...
import com.eventticket.service.admin.TtbTicketService;
...
public class TtbTicketController {

    private final TtbTicketService ttbTicketService;

    // Xem danh sách toàn bộ vé điện tử
    @GetMapping("/all")
    public ResponseEntity<List<G8_ticket>> getAllTickets() {
        return ResponseEntity.ok(ttbTicketService.getAllTicketsForAdmin());
    }
    ...
    public ResponseEntity<G8_ticket> getByQrCode(@PathVariable String qrCode) {
        return ResponseEntity.ok(ttbTicketService.findTicketByQrCode(qrCode));
    }
    ...
    public ResponseEntity<List<G8_ticket>> getByCheckInStatus(@PathVariable Boolean checkInStatus) {
        return ResponseEntity.ok(ttbTicketService.filterTicketsByCheckInStatus(checkInStatus));
    }
    ...
    public ResponseEntity<?> processTicketCheckIn(@PathVariable String qrCode) {
        try {
            ...
            G8_ticket checkedInTicket = ttbTicketService.processCheckInLogic(qrCode);
