...
import com.eventticket.service.admin.TtbPromotionService;
...
public class TtbPromotionController {

    // Khai báo private final thay vì @Autowired (Chuẩn xịn của Spring Boot hiện nay)
    private final TtbPromotionService ttbPromotionService;
    ...
    public ResponseEntity<List<G8_promotion>> getPromotions(
        ...
        if (keyword == null && isActive == null) {
            return ResponseEntity.ok(ttbPromotionService.getAllPromotions());
        }
        return ResponseEntity.ok(ttbPromotionService.searchAndFilterPromotions(keyword, isActive));
    }
    ...
    public ResponseEntity<G8_promotion> getPromotionById(@PathVariable Integer id) {
        return ResponseEntity.ok(ttbPromotionService.getPromotionById(id));
    }
    ...
    public ResponseEntity<G8_promotion> createPromotion(@RequestBody G8_promotion promotion) {
        return ResponseEntity.ok(ttbPromotionService.createPromotion(promotion));
    }
    ...
    public ResponseEntity<G8_promotion> updatePromotion(
        ...
        return ResponseEntity.ok(ttbPromotionService.updatePromotion(id, promotionDetails));
    }
    ...
    public ResponseEntity<G8_promotion> togglePromotionStatus(@PathVariable Integer id) {
        return ResponseEntity.ok(ttbPromotionService.togglePromotionStatus(id));
    }
}